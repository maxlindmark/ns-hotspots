...
# 4. I know also merge Norwegian with ICES data
...
# Now pull the CA, HL, and HH from the datrasraw object so that I can combine it with the norwegian data (which comes in csv)
dat_CA <- dat$CA
dat_HL <- dat$HL
dat_HH <- dat$HH

head(as.data.frame(no_HL))
head(as.data.frame(dat_HL))
sort(names(no_HL))
sort(names(dat_HL))

# Harmonize column names
no_CA <- no_CA |>
  rename(IndWgt = IndividualWeight,
         LngtClas = LengthClass) |>
  mutate(Quarter = as.factor(Quarter),
         DoorType = as.factor(DoorType),
         Year = as.factor(Year),
         MaturityScale = as.factor(MaturityScale),
         Survey = as.factor(Survey))

CA <- bind_rows(no_CA |> mutate(source = "NO"),
                dat_CA |> mutate(source = "DAT"))

hist(no_HL$NumberAtLength)
hist(dat_HL$HLNoAtLngt)

summary(no_HL$NumberAtLength)
summary(dat_HL$HLNoAtLngt)

summary(no_HL$LengthClass)
unique(no_HL$LengthCode)
summary(dat_HL$LngtCm)

no_HL <- no_HL |>
  rename(HLNoAtLngt = NumberAtLength,
         LngtCm = LengthClass) |>
  mutate(Quarter = as.factor(Quarter),
         DoorType = as.factor(DoorType),
         Year = as.factor(Year),
...
HL <- bind_rows(no_HL |> mutate(source = "NO"),
                dat_HL |> mutate(source = "DAT"))
...
no_HH <- no_HH |>
  mutate(Quarter = as.factor(Quarter),
         ...
         Day = as.integer(Day),
         DepthStratum = as.factor(DepthStratum),
         DayNight = as.factor(DayNight),
         Rigging = as.factor(Rigging),
         Year = as.factor(Year),
         Survey = as.factor(Survey))

HH <- bind_rows(no_HH |> mutate(source = "NO"),
                dat_HH |> mutate(source = "DAT"))
...
unique(dat$quarter_f)

ggplot(dat, aes(as.numeric(as.character(quarter_f)))) +
  geom_histogram()

ggplot(dat, aes(as.numeric(as.character(month_f)))) +
  geom_histogram()
...
tic()
m1 <- sdmTMB(
  biomass_kg ~ 0 + year_f + depth_sc + depth_sc2 + gear_f + quarter_f + (1|ship_f),
  data = dat,
  mesh = mesh,
  family = tweedie(),
...
pq <- pred_grid |>
  filter(year == 2000) |> 
  replicate_df("quarter_f", as.factor(c(1:4)))
...
tic()
m2 <- sdmTMB(
  biomass_kg ~ 0 + year_f + depth_sc + depth_sc2 + gear_f + month_f + (1|ship_f),
  data = dat,
  mesh = mesh,
  #family = delta_gamma(type = "poisson-link"),
  family = tweedie(),
